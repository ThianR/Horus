package com.horus.asistencia.sync.controller;

import com.horus.asistencia.marcas.controller.MarcacionController.MarcacionDto;
...
public class SyncController {
    ...
    private final com.horus.asistencia.biometria.service.PaqueteEmbeddingsService paqueteService;
    ...
    public ResponseEntity<?> descargarEmbeddings(@PathVariable Long sedeId,
        ...
        List<com.horus.asistencia.biometria.model.PerfilBiometrico> perfiles = perfilRepository.findAllByEmpresaId(empresaService.getEmpresaDefault().getId());

        return ResponseEntity.ok(perfiles.stream()
                .filter(p -> p.getVersion() > version)
                .map(p -> {
                    List<byte[]> embeddings = p.getMuestras().stream()
                            .map(com.horus.asistencia.biometria.model.MuestraBiometrica::getEmbedding)
                            .toList();
