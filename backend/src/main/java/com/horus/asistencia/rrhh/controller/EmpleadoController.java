...
import com.horus.asistencia.rrhh.dto.AsignacionMasivaDto;
...
public class EmpleadoController {
    ...
    private final com.horus.asistencia.rrhh.repository.EmpleadoSedeHabilitadaRepository sedeHabilitadaRepository;
    private final com.horus.asistencia.reportes.service.ImportacionMasivaService importacionMasivaService;
    private final com.horus.asistencia.organizacion.service.EmpresaService empresaService;
    private final com.horus.asistencia.turnos.repository.AsignacionTurnoRepository asignacionTurnoRepository;
    private final com.horus.asistencia.turnos.repository.TurnoPlantillaRepository turnoPlantillaRepository;
    private final com.horus.asistencia.biometria.repository.PerfilBiometricoRepository perfilBiometricoRepository;
    private final com.horus.asistencia.marcas.repository.MarcacionEventoRepository marcacionEventoRepository;
    private final com.horus.asistencia.organizacion.repository.SedeRepository sedeRepository;
    ...
    public ResponseEntity<List<Empleado>> listarTodos() {
        ...
        List<com.horus.asistencia.turnos.model.AsignacionTurno> asignaciones = asignacionTurnoRepository.findAllByEmpresaId(empresaService.getEmpresaDefault().getId())
                .stream()
                .filter(a -> !a.getFechaInicio().isAfter(hoy)
                        && (a.getFechaFin() == null || !a.getFechaFin().isBefore(hoy)))
                .toList();

        List<com.horus.asistencia.rrhh.model.EmpleadoSedeHabilitada> sedesHabilitadas = sedeHabilitadaRepository
                .findAll().stream()
                .filter(com.horus.asistencia.rrhh.model.EmpleadoSedeHabilitada::isActivo)
                .toList();

        List<com.horus.asistencia.biometria.model.PerfilBiometrico> perfiles = perfilBiometricoRepository.findAll();
        ...
        for (Empleado emp : empleados) {
            ...
            for (com.horus.asistencia.turnos.model.AsignacionTurno a : asignaciones) {
    ...
    public ResponseEntity<?> crear(@RequestBody Empleado empleado) {
        ...
        com.horus.asistencia.identidad.model.Usuario nuevoUsuario = new com.horus.asistencia.identidad.model.Usuario();
        ...
        try {
            nuevoUsuario.setRol(com.horus.asistencia.identidad.model.Usuario.Rol.valueOf(rolReq));
        } catch (IllegalArgumentException e) {
            nuevoUsuario.setRol(com.horus.asistencia.identidad.model.Usuario.Rol.EMPLEADO);
        }
    ...
    public ResponseEntity<?> actualizar(@PathVariable Long id, @RequestBody Empleado empleado) {
        ...
        if (empActual.getUsuario() != null && empleado.getRolSistema() != null) {
            com.horus.asistencia.identidad.model.Usuario usu = empActual.getUsuario();
            try {
                usu.setRol(com.horus.asistencia.identidad.model.Usuario.Rol.valueOf(empleado.getRolSistema()));
    ...
    private void gestionarSede(Empleado empleado, Long sedeId) {
        if (sedeId != null) {
            java.util.Optional<com.horus.asistencia.rrhh.model.EmpleadoSedeHabilitada> habilitacionOpt = sedeHabilitadaRepository
            ...
            if (habilitacionOpt.isEmpty() || !habilitacionOpt.get().getSede().getId().equals(sedeId)) {
                if (habilitacionOpt.isPresent()) {
                    com.horus.asistencia.rrhh.model.EmpleadoSedeHabilitada prev = habilitacionOpt.get();
                ...
                sedeRepository.findById(sedeId).ifPresent(sede -> {
                    com.horus.asistencia.rrhh.model.EmpleadoSedeHabilitada nuevaHabilitacion = new com.horus.asistencia.rrhh.model.EmpleadoSedeHabilitada();
    ...
    public ResponseEntity<com.horus.asistencia.rrhh.dto.ImportacionResultDto> importarEmpleados(@RequestParam("file") MultipartFile file)
            throws java.io.IOException {
        com.horus.asistencia.rrhh.dto.ImportacionResultDto result = importacionMasivaService.importarEmpleadosYSedes(file.getInputStream());
