package com.horus.asistencia.marcas.controller;
...
import com.horus.asistencia.motor.service.MotorAsistenciaService;
import com.horus.asistencia.rrhh.model.Empleado;
import com.horus.asistencia.rrhh.repository.EmpleadoRepository;
import com.horus.asistencia.integraciones.service.WebhookDispatcherService;
...
public class MarcacionController {
    ...
    private final com.horus.asistencia.marcas.repository.MarcacionIntentoRepository intentoRepository;
    private final EmpleadoRepository empleadoRepository;
    private final MotorAsistenciaService motorService;
    private final com.horus.asistencia.biometria.repository.PerfilBiometricoRepository perfilRepository;
    private final com.horus.asistencia.biometria.service.BiometriaService biometriaService;
    ...
    public ResponseEntity<?> identificarYRegistrar(
        ...
        try {
            ...
            com.horus.asistencia.organizacion.model.Sede sede = null;
            if (sedeId != null) {
            ...
            List<com.horus.asistencia.biometria.service.BiometriaService.PerfilCandidato> todosCandidatos = obtenerCandidatosPorEmpresa(empresaId);
            ...
            if (sede != null) {
                ...
                List<com.horus.asistencia.biometria.service.BiometriaService.PerfilCandidato> candidatosSede = todosCandidatos.stream()
                ...
                if (empleadoId == null) {
                    List<com.horus.asistencia.biometria.service.BiometriaService.PerfilCandidato> candidatosResto = todosCandidatos.stream()
                            .filter(c -> !empleadosEnSede.contains(c.empleadoId())).toList();
    ...
    private ResponseEntity<?> procesarRegistro(MarcacionDto dto) {
        ...
        com.horus.asistencia.marcas.model.MarcacionIntento intento = new com.horus.asistencia.marcas.model.MarcacionIntento();
    ...
    private List<com.horus.asistencia.biometria.service.BiometriaService.PerfilCandidato> obtenerCandidatosPorEmpresa(Long empresaId) {
        List<com.horus.asistencia.biometria.service.BiometriaService.PerfilCandidato> lista = new java.util.ArrayList<>();
        
        perfilRepository.findAllByEmpresaId(empresaId).stream()
                .filter(com.horus.asistencia.biometria.model.PerfilBiometrico::isActivo)
                .forEach(p -> {
                    for (com.horus.asistencia.biometria.model.MuestraBiometrica muestra : p.getMuestras()) {
                        if (muestra.getEmbedding() != null) {
                            float[] emb = bytesToFloats(muestra.getEmbedding());
                            lista.add(new com.horus.asistencia.biometria.service.BiometriaService.PerfilCandidato(
                                    p.getEmpleado().getId(),
                                    emb));
                        }
                    }
